...
```
$ docker run --name some-postgres -e POSTGRES_PASSWORD=mysecretpassword -d metainnovative/postgres
```
...
### or via `psql`
````
$ docker run -it --rm --link some-postgres:postgres metainnovative/postgres psql -h postgres -U postgres
psql (9.5.0)
Type "help" for help.
...
```
FROM metainnovative/postgres:latest
ENV LANG de_DE.utf8
```
